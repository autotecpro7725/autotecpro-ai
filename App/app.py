...
if assistant == "⚙️ Admin Panel":
    ...
    with tab1:
        ...
        st.caption(
            "Deleting a user permanently removes the account and disables its active login sessions."
        )
        ...
        selected_delete_username = st.selectbox(
            ...
            options=["— Select a user —"] + delete_user_options,
            key="admin_delete_user_select"
        )

        delete_confirmation = st.checkbox(
            "I understand this action cannot be undone.",
            key="admin_delete_user_confirmation"
        )
        ...
        if st.button(
            ...
            if selected_delete_username == "— Select a user —":
            ...
            elif selected_delete_username == st.session_state.username:
                st.error(
                    "You cannot delete the account you are currently logged in with. "
                    "Log in using another administrator account first."
                )
                ...
                selected_user = selected_user_rows[0] if selected_user_rows else {}

                if str(selected_user.get("role", "")).lower() == "admin":
                    active_admins = [
                        user
                        for user in users
